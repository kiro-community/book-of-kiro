...
这是因为 Kiro 会把一些复杂命令识别为危险命令，需要人为确认，详见[官方文档](https://kiro.dev/docs/chat/terminal/#dangerous-patterns)

比如：设置了信任 `python *`，但是在执行 `python -c "print(';')"` 时，因为命令中包含分号 `;` ，所以会被识别为危险命令，需要人为确认。

如果要绕过此限制，需要在信任命令中配置 `*`，即信任所有命令，这样 Kiro 就不会弹出确认框了，但是需要注意此操作存在安全风险。
