...
### 🌐 **Kiro Web**
基于浏览器的 AI 开发环境，提供 Vibe 模式（交互式协作）和 Autonomous 模式（自主任务处理）两种工作方式。在安全沙盒中运行，支持 GitHub/GitLab 集成和自动 PR 创建。目前正在向 Kiro Pro、Pro+ 和 Power 用户推出预览版。
...
- **灵活的自主级别**：Kiro IDE 提供 Supervised 和 Autopilot 两种模式控制 Agent 自主级别；Kiro Web 则提供 Vibe 模式（交互式协作）和 Autonomous 模式（自主任务处理）
