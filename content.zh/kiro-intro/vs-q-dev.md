...
Kiro 和 Amazon Q Developer 都是 AWS 出品的 AI 开发工具。Kiro 是一个完整的 AI 开发平台（包含 IDE、CLI 和 Kiro Web），而 Amazon Q Developer 主要以 IDE 插件形式提供。从整体用户体验而言，Kiro 提供更全面的解决方案，Amazon Q Developer 插件则支持不同的 IDE，用户无需更换原有 IDE。以下是主要区别：
...
**Kiro** 是一个完整的 AI 开发平台，包含 IDE、CLI 和 Kiro Web 三大组件，专为 AI 辅助开发而从头构建。**Amazon Q Developer** 主要作为插件/扩展在现有开发环境（如 Visual Studio Code、JetBrains、Eclipse、Visual Studio）中工作。
