...
Kiro 和 Amazon Q Developer 都是 AWS 出品的 AI 開發工具。Kiro 是一個完整的 AI 開發平臺（包含 IDE、CLI 和 Autonomous Agent），而 Amazon Q Developer 主要以 IDE 外掛形式提供。從整體使用者體驗而言，Kiro 提供更全面的解決方案，Amazon Q Developer 外掛則支援不同的 IDE，使用者無需更換原有 IDE。以下是主要區別：
...
**Kiro** 是一個完整的 AI 開發平臺，包含 IDE、CLI 和 Autonomous Agent 三大元件，專為 AI 輔助開發而從頭構建。**Amazon Q Developer** 主要作為外掛/擴充套件在現有開發環境（如 Visual Studio Code、JetBrains、Eclipse、Visual Studio）中工作。

主要區別：

- **Kiro**：完整的獨立 AI 開發平臺，提供全方位的開發體驗
...
Kiro 作為完整平臺提供了 Amazon Q Developer 外掛無法匹敵的功能：
